...
import { UserPermissionsResponseDto } from '../../dto/user-response.dto';
  ...
  implements IQueryHandler<GetUserPermissionsQuery>
{
  constructor(
    ...
    this.logger.setContext(GetUserPermissionsHandler.name);
  }
  ...
  async execute(
    ...
    try {
      ...
      return { permissions } as UserPermissionsResponseDto;
    } catch (error) {
      if (error.status) throw error;
      this.logger.error('Failed to get user permissions', error.stack);
